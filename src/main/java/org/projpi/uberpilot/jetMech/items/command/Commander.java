package org.projpi.uberpilot.jetMech.items.command;
        ...
        if(args.length >= 1)
        {
            if(args[0].equalsIgnoreCase("give") || args[0].equalsIgnoreCase("g"))
            {
                if(args.length == 3)
                {
                    if (sender.hasPermission("jetItems.give.self"))
                    {
                        ...
                        {
                            if (JetItems.items.containsKey(args[1].toLowerCase()))
                            {
                                try
                                {
                                    ItemStack is = JetItems.items.get(args[1].toLowerCase());
            ...
            else if(args[0].equalsIgnoreCase("list") || args[0].equalsIgnoreCase("l"))
            {
                if(sender.hasPermission("jetItems.list"))
                {
                    ...
                    for(Map.Entry<String, ItemStack> entry : JetItems.items.entrySet())
                    {
